...
In this paper we apply the before-mentioned methods for  results numerical of numerical experiments, and discuss the other methods, which are difficult to assess in the experiments.
...
Geometrically the computational domain is a rectangular trapeze, with the wavemaker located at the left wall, as in the experiments~\cite{2013ScolanErmanyukDauxois,2016BrouzetSibgatullinScolanErmanyukDauxois}.
...
The results of numerical simulation were approximated at a regular rectangular grid with the help of openFoam functionObject and processed by python scripts. Hydrodynamic fields were visualised with the help of open-source, multi-platform data analysis and visualization platform paraview \cite{paraview}. 
...
\centering
    ...
    \begin{minipage}[t]{0.45\textwidth}
        ...
        \label{fig:turbAttr}
    \end{minipage}
    \caption{Horizontal component of velocity field with internal wave focusing which leads from attractor forming (from 0 to 10 wavemaker periods) to stability (from 10 to 50 oscilation periods) and further (from 50 and belong) to attractor destruction}
    \label{fig:2dAttr}
\end{figure}
